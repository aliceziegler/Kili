...
##create column in dat_SR from luidich with 1 for disturbed and 0 for natural
dat_path <- paste0("../data/", sub)
dat_SR_path <- "dat_SR"

dat_SR <- get(load(paste0(dat_path,dat_SR_path, ".RData")))
dat_SR$dstrb = ifelse(dat_SR$luidich == "Disturbed", 1, 0)
dat_SR <- dat_SR[,c(1:12, 41, 13:40)]
save(dat_SR, file = paste0(dat_path, "dat_SR.RData"))
...
# ########################################################################################
# get_lidar <- F
# requ_ldr <- c("kili_campaign1_lidar_classified_2015", "kili_campaign2_lidar_classified_2016")
# ########################################################################################
