...
gam_prdct_df_SR <- gam_prdct_df[,!grepl("resid", colnames(gam_prdct_df))]
...
gam_resid <- data.frame(plotID = gam_prdct_df_SR$plotID, plotUnq = gam_prdct_df_SR$plotUnq)
...
for(i in colnames(prdct_res)[3:ncol(prdct_res)]){
  ...
  gam_resid$tmp <- gam_prdct_df_SR[,match] + prdct_res[,i]
  colnames(gam_resid)[which(colnames(gam_resid) == "tmp")] <- i
}
  
#   if(i %in% colnames(gam_prdct_df_SR) & i %in% colnames(res_tbl)){
#     gam_resid$tmp <- gam_prdct_df_SR[,i]+res_tbl[,i]
#     colnames(gam_resid)[which(colnames(gam_resid) == "tmp")] <- i
#   }
# }
# # i <- 3
...
SR_tbl <- mrg_tbl[, which(colnames(mrg_tbl) %in% colnames(gam_prdct_df_SR))]

stats_gam_resid <- data.frame(resp = colnames(gam_resid)[3:ncol(gam_resid)], R2 = NA, RMSE = NA)
for (i in colnames(gam_resid)[3:ncol(gam_resid)]){
  match <- substr(i,6, nchar(i)-5)
  stat <- postResample(gam_resid[,which(colnames(gam_resid) == i)], SR_tbl[,which(colnames(SR_tbl) == match)])
  stats_gam_resid[which(stats_gam_resid$resp == i), which(colnames(stats_gam_resid) == "R2")] <- stat[2]
  stats_gam_resid[which(stats_gam_resid$resp == i), which(colnames(stats_gam_resid) == "RMSE")] <- stat[1]
