...
for (i in c(2:ncol(mrg_obs))){
  dat <- data.frame("elevation" = elev$elevation, "response" = mrg_obs[,i])
  mod <- mvr(response ~ elevation, data = dat)
...
#stats gam
stats_gam <- data.frame(resp = colnames(mrg_obs)[2:ncol(mrg_obs)], RMSE = NA, R2 = NA, sd = NA, RMSE_sd = NA, type = "gam")
...
#stats_pls
stats_pls <- data.frame(resp = colnames(mrg_obs)[2:ncol(mrg_obs)], RMSE = NA, R2 = NA, sd = NA, RMSE_sd = NA, type = "pls")
for (i in colnames(mrg_obs)[2:ncol(mrg_obs)]) {
...
mod_df <- rbind(stats_gam, stats_pls)
