####to do: in funktion schreiben! urspr. aus script plots_model_list
...
for (i in seq(mod_all)){
  ...
  #plot(mod_all[[i]]) # für ncomp = 1 nicht machbar
  print(mod_all[[i]])
}
...
########################################################################################
#selectedvars from ffs
########################################################################################
sel_vars <- c()
for (i in seq(mod_all)){
  sel_vars_i <- mod_all[[i]]$selectedvars
  sel_vars <- c(sel_vars, sel_vars_i)
}
sel_vars_frq <- table(sel_vars)
sel_vars_frq <- sel_vars_frq[order(sel_vars_frq, decreasing = T)]
sel_vars_top10 <- sel_vars_frq[which(sel_vars_frq >= 10)]
#plot how often predictors where chosen
barplot(sel_vars_frq, las = 2, cex.names= 0.5)
####nur Prediktoren, die mehr als 10 mal ausgewählt wurden 
########################macht es hier eventuell sinn die Säulen nach Var imp einzufärben? 
###############macht es generell Sinn die Varimp mit dem R² zu skalieren? in eine Art verlässlichkeits/Brauchbarkeitsmaß?
barplot(sel_vars_top10, las = 2, cex.names= 1)
axis(2,at=seq(1,range(sel_vars_top10)[2],by=1),labels=FALSE)
########################################################################################
#Variable importance
########################################################################################
imp_top5 <- c()
...
for (i in seq(mod_all)){
  ...
  df <- as.data.frame(varImp(mod_all[[i]])$importance)
