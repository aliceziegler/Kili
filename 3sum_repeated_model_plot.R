...
for (i in seq(nrow(stats))){
  troph_nm <- strsplit((as.character(stats$resp)[i]), split = "_")
  stats$troph[i] <- troph_nm[[1]][2]
}
...
for (j in plots){
  pdf(file = paste0(outpath, "boxplot_", j, ".pdf"), width = 12, height = 12)
    plot_trop(df = stats, var = j, resp_title = "trophic summary")
    dev.off()
  }
...
# #testing
# plot_trop_mean <- function(df, var, names, resp_title, smmry = "resp", path = outpath, 
...
# 
# 
# df_mean_tmp <- df_mean[which(df_mean$resp %in% variations[[2]]),]
...
#           resp_title = paste0("mean_Rsquared_", names(variations)[2]))
