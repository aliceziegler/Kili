...
scl_lst <- lapply(df[,c(1:ncol(df))], function(i){
  if (class(i) == "numeric"){
...
df_scl <- do.call(data.frame, scl_lst)
...
mod_pls <- train(df_scl[, c(1:(ncol(df_scl)-1))], df_scl[,c(ncol(df_scl))], method = "kernelpls", 
