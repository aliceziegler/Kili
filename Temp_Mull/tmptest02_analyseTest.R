...
#loop over all resposes
#loop over all runs
# strsplit for response and run name
# get outs plots for each run
...
save(mod_lst, file = paste0(inpath, "/pls_model_list_all_lst.RData")) ####liste alles modelle anders sortiert!
...
prediction_rep <- lapply(names(mod_lst), function(y){
  if (length(mod_lst[[y]]) != 0){
    ...
    prediction_run <- lapply(seq(length(mod_lst[[y]])), function(z){
      print(z)
      outs <- outs_lst[[grep(paste0(z, "$"), names(outs_lst))]]$plotID #1$ means search for 1 at end of line
